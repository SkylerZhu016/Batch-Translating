...
> **状态：仍在调试阶段。** 本项目仍在调试阶段，部分功能无法按预期运行，请等待开发者后续维护。
...
```sh
node scripts/ci/stage-rag-service.mjs   # 只暂存白名单 RAG Python 源码（不含模型/venv/缓存）
corepack pnpm batch:desktop:build       # → Batch Translating_0.2.0_x64-setup.exe
```

CI 在 `.github/workflows/batch-windows-build.yml` 中用固定 Tauri CLI 执行同一构建：产出 NSIS 前先做安装包内容扫描（不携带模型、venv、缓存、token、数据库或测试），且不上传裸桌面 exe。本机缺 `signtool` 只是签名 warning，不影响 SEA 注入与启动。

`.github/workflows/batch-release.yml` 是独立的、需人工触发的门控发布流程 —— 不会因 push/tag 自动触发。
