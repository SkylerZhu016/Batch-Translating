// apps/batch-translating-web/test/daemon-client.test.ts
...
const WIRE_SESSION = {
  ...
  metadata: {
    cwd: 'C:\\Books',
    batchTranslation: { stage: 'preflight' },
  },
  agent_config: { model: '' },
  usage: {
    input_tokens: 0,
...
describe('DaemonKimiWebApi translation session contracts', () => {
  beforeEach(() => {
    vi.stubGlobal('fetch', vi.fn());
  });
  ...
  it('creates a profiled session with complete translation metadata', async () => {
    vi.mocked(fetch).mockResolvedValue(envelope(WIRE_SESSION));

    const session = await createApi().createSession({
      title: 'Book',
      cwd: 'C:\\Books',
      metadata: { batchTranslation: { stage: 'preflight' } },
      agentProfile: 'batch-translator',
    });
    ...
    expect(session.metadata).toEqual(WIRE_SESSION.metadata);
  });

  it('patches and permanently deletes translation project metadata', async () => {
    vi.mocked(fetch)
      .mockResolvedValueOnce(envelope(WIRE_SESSION))
      .mockResolvedValueOnce(envelope({ deleted: true }));

    await createApi().updateSession('sess/1', {
    ...
    expect(vi.mocked(fetch).mock.calls[1]?.[0]).toBe(
      'http://daemon.test/api/v1/sessions/sess%2F1',
    );
    expect(vi.mocked(fetch).mock.calls[1]?.[1]).toMatchObject({ method: 'DELETE' });
  });

  it('maps the profile and session tool denylist on prompt submission', async () => {
    vi.mocked(fetch).mockResolvedValue(
      envelope({ prompt_id: 'prompt_1', user_message_id: 'message_1', status: 'running' }),
    );

    await createApi().submitPrompt('sess_1', {
      content: [{ type: 'text', text: 'Start preflight' }],
      profile: 'batch-translator',
      disabledTools: ['Edit', 'Grep'],
    });
  ...
  it('round-trips tools and identity through the config mapper', async () => {
    const wireConfig = {
      providers: {},
      tools: { enabled: ['Read', 'Write', 'Bash', 'AgentSwarm'] },
      identity: { name: 'Batch Translating', slug: 'batch-translating' },
    };
    vi.mocked(fetch).mockResolvedValue(envelope(wireConfig));

    const config = await createApi().setConfig({
    ...
    expect(config.tools).toEqual(wireConfig.tools);
    expect(config.identity).toEqual(wireConfig.identity);
  });
});
