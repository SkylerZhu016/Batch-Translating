<!-- apps/batch-translating-web/src/components/settings/ProviderManager.vue -->
...
const emit = defineEmits<{
  add: [input: { id: string; type: string; apiKey?: string; baseUrl?: string; defaultModel?: string; models: Array<{ model: string; maxContextSize?: number }> }];
  refresh: [id: string];
  delete: [id: string];
  editModels: [id: string];
  close: [];
}>();
...
const PROVIDER_TYPES = ['openai', 'anthropic', 'google-genai', 'vertexai'];
...
function submitAdd(): void {
  ...
  if (!addForm.defaultModel.trim()) {
    addError.value = t('providers.modelRequired');
    return;
  }
  addError.value = '';
  emit('add', {
    ...
    models: [{ model: addForm.defaultModel.trim(), maxContextSize: 200_000 }],
  });
  showAddForm.value = false;
}
        ...
        <template v-else>
          <div v-for="p in providers" :key="p.id" class="prov-row">
            ...
            <div class="prov-actions">
              <Tooltip :text="t('providers.editModelsTitle', { id: p.id })">
                <Button variant="secondary" size="sm" @click="emit('editModels', p.id)">{{ t('providers.editModels') }}</Button>
              </Tooltip>
      ...
      <div v-if="!unavailable" class="add-section">
        ...
        <template v-else>
          <div class="add-form">
            ...
            <Field label="模型名 (Model)" hint="必填：填一个上游模型 id，保存后可再补充">
              <Input
