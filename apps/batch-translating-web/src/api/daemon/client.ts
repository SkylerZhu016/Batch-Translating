// apps/batch-translating-web/src/api/daemon/client.ts
...
export class DaemonKimiWebApi implements KimiWebApi {
  ...
  async createSession(input: {
    ...
    if (input.agentProfile !== undefined) body['agent_profile'] = input.agentProfile;
  ...
  async updateSession(
    ...
    if (input.title !== undefined) body['title'] = input.title;
    if (input.cwd !== undefined || input.metadata !== undefined) {
      const metadata: Record<string, unknown> = { ...input.metadata };
      if (input.cwd !== undefined) metadata['cwd'] = input.cwd;
  ...
  async deleteSession(sessionId: string): Promise<void> {
    await this.http.delete(`/sessions/${encodeURIComponent(sessionId)}`);
  }
  ...
  async addProvider(input: {
    ...
    // unknown-context budget at runtime).
    const body: Record<string, unknown> = {
      id: input.id,
      type: input.type,
  ...
  /** Replace a provider in one save (PUT /v1/providers/{id}) — rebuilds its
   *  model aliases from `models`. `apiKey` is tri-state on the wire: absent
  ...
  async replaceProvider(id: string, input: {
    ...
    if (input.baseUrl !== undefined) body['base_url'] = input.baseUrl;
    if (input.defaultModel !== undefined) body['default_model'] = input.defaultModel;
    const data = await this.http.put<WireProvider>(
  ...
  /** Gracefully shut down the daemon. The HTTP call is best-effort: the server
   *  tears down right after replying, so a dropped connection is normal. */
