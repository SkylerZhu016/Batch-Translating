...
export interface AppSession {
  ...
  lastPrompt?: string;
  cwd: string;
  /** Complete daemon metadata, including translation project state. */
...
export interface PromptSubmission {
  ...
  profile?: string;
  /** Session-scoped tool denylist applied after profile binding. */
...
export interface KimiWebApi {
  ...
  createSession(input: { title?: string; cwd?: string; model?: string; workspaceId?: string; metadata?: Record<string, unknown>; agentProfile?: string }): Promise<AppSession>;
  /** Fetch one session by id (deep links beyond the first listSessions page). */
  getSession(sessionId: string): Promise<AppSession>;
  updateSession(sessionId: string, input: { title?: string; cwd?: string; metadata?: Record<string, unknown>; model?: string; permissionMode?: string; planMode?: boolean; swarmMode?: boolean; goalObjective?: string; goalControl?: 'pause' | 'resume' | 'cancel'; thinking?: string }): Promise<AppSession>;
  /** Permanently remove a session and its persisted project state. */
  ...
  /** Gracefully stop the daemon (used by the exit-app flow). */
