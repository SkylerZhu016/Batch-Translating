// apps/batch-translating-web/src/composables/useKimiWebClient.ts
...
function loadPermissionFromStorage(): PermissionMode {
  ...
  // Fresh installs default to auto: this app's users don't write code, and an
  // unattended approval prompt would stall the agent forever. The setting UI
  // can still switch back to manual/yolo.
  return 'auto';
}
...
/** Shared empty array for messagesForSession misses (stable reference). */
const EMPTY_MESSAGES: readonly AppMessage[] = [];
...
/** Domain-facing session list. Keep this separate from the legacy `sessions`
...
const translationCoordinator = useTranslationCoordinator({
  ...
  deleteSession: async (sessionId) => {
    await getKimiWebApi().deleteSession(sessionId);
  },
  ...
  uploadFile: async (file, name) => {
    const uploaded = await getKimiWebApi().uploadFile({ file, name });
    return {
      fileId: uploaded.id,
      kind: 'file',
      name: uploaded.name,
...
export function useKimiWebClient() {
  ...
  return {
    ...
    activeAppTasks,
    /** Live message stream for one session (drives the run-progress debug
     *  view; includes tool calls and results as they stream in). */
    messagesForSession: (sessionId: string) =>
      rawState.messagesBySession[sessionId] ?? EMPTY_MESSAGES,
    todos,
    goal,
    swarms,
    ...
    setDayTheme: appearance.setDayTheme,
    nightTheme: appearance.nightTheme,
    setNightTheme: appearance.setNightTheme,
    ...
    replaceProvider: modelProvider.replaceProvider,
