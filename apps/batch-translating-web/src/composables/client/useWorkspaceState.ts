// apps/batch-translating-web/src/composables/client/useWorkspaceState.ts
...
export function useWorkspaceState(rawState: ExtendedState, deps: UseWorkspaceStateDeps) {
  ...
  async function submitPromptInternal(
    ...
    try {
      ...
      const result = await api.submitPrompt(sid, {
        ...
        // Caller-supplied mode wins (translation stages force 'auto' so tool
        // calls never stall on an unattended approval prompt); otherwise fall
        // back to the user's global permission mode.
  ...
  /** Gracefully stop the daemon (the "exit app" flow). Best-effort: the server
   *  tears down right after replying, so a dropped connection is expected. */
  async function shutdown(): Promise<void> {
    await getKimiWebApi().shutdown();
  }
