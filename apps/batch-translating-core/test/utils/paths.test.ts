...
describe('getDataDir', () => {
  it('returns ~/.batch-translating when KIMI_CODE_HOME is not set', () => {
...
describe('getLogDir', () => {
  it('returns <dataDir>/logs', () => {
    expect(getLogDir()).toBe(join(homedir(), '.batch-translating', 'logs'));
  });
...
describe('getBinDir', () => {
  it('returns <dataDir>/bin', () => {
    expect(getBinDir()).toBe(join(homedir(), '.batch-translating', 'bin'));
  });
...
describe('getUpdateStateFile', () => {
  it('returns <dataDir>/updates/latest.json', () => {
    expect(getUpdateStateFile()).toBe(join(homedir(), '.batch-translating', 'updates', 'latest.json'));
  });
...
describe('getUpdateInstallStateFile', () => {
  it('returns <dataDir>/updates/install.json', () => {
    expect(getUpdateInstallStateFile()).toBe(
      join(homedir(), '.batch-translating', 'updates', 'install.json'),
    );
  });
...
describe('getInputHistoryFile', () => {
  it('returns <dataDir>/user-history/<md5(workDir)>.jsonl', () => {
    ...
    expect(getInputHistoryFile(workDir)).toBe(
      join(homedir(), '.batch-translating', 'user-history', `${hash}.jsonl`),
    );
  });
