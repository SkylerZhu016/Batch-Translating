 ...
 * Run: pnpm -C apps/batch-translating-core exec vitest run test/cli/run-prompt.test.ts
 */
  ...
  return {
    ...
    runV2Print: vi.fn(
      async (
        ...
        if (outputFormat === 'stream-json') {
          ...
          stdout.write(
            `${JSON.stringify({
              ...
              command: 'batch-translating -r ses_prompt',
              content: 'To resume this session: batch-translating -r ses_prompt',
            })}\n`,
          );
          return;
        }
        ...
        stderr.write('To resume this session: batch-translating -r ses_prompt\n');
      },
    ),
...
describe('runPrompt', () => {
  ...
  it('creates a fresh auto-permission session and streams assistant output to stdout', async () => {
    ...
    expect(stderr.text()).toBe('To resume this session: batch-translating -r ses_prompt\n');
  ...
  it('does not create a first-launch telemetry identity in prompt mode', async () => {
  ...
  it('formats thinking and assistant output as transcript blocks', async () => {
    ...
    expect(stderr.text()).toBe(
      '• The user wants an exact reply.\n  No tools are needed.\n\nTo resume this session: batch-translating -r ses_prompt\n',
    );
  ...
  it('writes stream-json output as assistant JSONL with resume meta without transcript bullets', async () => {
    ...
    expect(stdout.text()).toBe(
      [
        ...
        '{"role":"meta","type":"session.resume_hint","session_id":"ses_prompt","command":"batch-translating -r ses_prompt","content":"To resume this session: batch-translating -r ses_prompt"}',
        '',
      ].join('\n'),
    );
    expect(stderr.text()).toBe('');
  ...
  it('throws when no default model is configured', async () => {
    ...
    await expect(
      ...
      'No model configured. Run `batch-translating` and use /login to sign in, then retry; or set default_model in config.toml.',
    );
