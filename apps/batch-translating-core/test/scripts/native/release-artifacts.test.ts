...
const windowsTarget = 'win32-test-zip-artifact';
...
const fakeWindowsBinary = resolve(
  appRoot,
  'dist-native/bin',
  windowsTarget,
  executableName,
);
const windowsLauncherEntries = [
...
describe('native release artifacts', () => {
  ...
  it('includes the non-technical launchers in Windows archives only', async () => {
    mkdirSync(resolve(appRoot, 'dist-native/bin', windowsTarget), { recursive: true });
    writeFileSync(fakeWindowsBinary, 'native Windows binary payload\n', { mode: 0o755 });
