...
export function executableName(platform = process.platform, env = process.env) {
  // Batch Translating ships the engine under its own name; the upstream
  // `kimi.exe` name stays the default so dev builds and the upstream native
  // release flow keep working unchanged.
  const override = env.KIMI_CODE_EXECUTABLE_NAME;
  if (override) {
    return override;
  }
