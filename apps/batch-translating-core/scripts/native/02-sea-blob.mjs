...
async function writeSeaConfig(target) {
  ...
  const config = {
    // Relative-to-appRoot main path: the SEA blob embeds this string verbatim
    // (it doubles as the embedded main's __filename). An absolute
    // build-machine path here would leak into the shipped binary, so it must
    // never be machine-specific.
    main: relative(appRoot, nativeJsBundlePath()).split('\\').join('/'),
