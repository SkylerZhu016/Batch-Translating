/**
 * Output rendering for `batch-translating -p` (print mode) — shared by the v1 driver
...
export function writeExperimentalVersion(
  ...
  stderr.write(`batch-translating version ${version}\n`);
}
...
export function writeResumeHint(
  ...
  const command = `batch-translating -r ${sessionId}`;
